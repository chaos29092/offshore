...
if (!function_exists('page_cache')) {
    function page_cache($page_slug)
    {
        $cache_key = 'page_'.$page_slug;
        $cache_data = Cache::get($cache_key);
        if (!$cache_data){
            $cache_data = Page::whereSlug($page_slug)->firstOrFail();
...
if (!function_exists('featured_products_cache')) {
    function featured_products_cache()
    {
        ...
        $cache_data = Cache::get($cache_key);
        if(!$cache_data){
            $cache_data = Product::whereFeatured(1)->orderBy('order', 'asc')->select('name','slug','image','order','featured')->take(4)->get();
            Cache::forever($cache_key,$cache_data);
        }
